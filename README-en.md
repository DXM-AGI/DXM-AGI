# Du Xiaoman AGI Lab
**Official Technical Sharing Account**

[中文](README.md) | English

---

Welcome to the official GitHub of Du Xiaoman AGI Lab! We are dedicated to advancing cutting-edge AI research, with a focus on the practical application of AGI technologies in the fintech industry and open-source sharing.

---

## 🔬 Research Directions

- **Large Language Models** - Model training, fine-tuning, alignment techniques
- **AI Agent** - Agent frameworks and applications
- **Financial AI** - Intelligent customer service, risk management, robo-advisory
- **RAG & Knowledge Enhancement** - Retrieval-Augmented Generation

---

## 🛠️ Open Source Projects

| Project | Description | Status |
|---------|-------------|--------|
| [FIRE-Bench](https://github.com/DXM-AGI/FIRE-Bench) | Financial Intelligence Evaluation Benchmark | ![Active](https://img.shields.io/badge/Status-Active-brightgreen) |

---

## 📄 Publications

| Paper | Venue | Year | Link |
|-------|-------|------|------|
| FIRE: A Comprehensive Benchmark for Financial Intelligence and Reasoning Evaluation | arXiv | 2026 | [Paper](https://arxiv.org/abs/2602.22273) |

---

## 🌐 Contact Us

- 🏢 Website: [Du Xiaoman](https://www.duxiaoman.com/)
- 📧 Email:
- 🐦 WeChat Official Account:

---

**⭐ If you find our projects helpful, please consider giving us a Star!**
